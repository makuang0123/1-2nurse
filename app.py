...
st.set_page_config(
    page_title="醫療網護理人員執登與調薪卡控系統", layout="wide"
)
...
ALL_CLINICS = [
    clinic for clinics in CLINIC_REGIONS.values() for clinic in clinics
]
CLINIC_TO_REGION = {
    clinic: region
    for region, clinics in CLINIC_REGIONS.items()
...
staff_df["符合原因"] = (
    staff_df["符合原因"].fillna("⚠️ 請選取原因").replace({"": "⚠️ 請選取原因"})
)
staff_df["符合資格"] = staff_df["符合原因"].map(get_compliance_status)

if not staff_df.empty and "院所" in staff_df.columns:
  staff_df["區域"] = staff_df["院所"].map(
      lambda x: CLINIC_TO_REGION.get(x, "屏東")
  )
  ...
  with st.sidebar.form("login_form"):
    ...
    if submit_button:
      if (
          ...
          and USER_CREDENTIALS[username_input]["password"] == password_input
      ):
...
def save_data(new_df):
  ...
  new_df["符合原因"] = (
      new_df["符合原因"].fillna("⚠️ 請選取原因").replace({"": "⚠️ 請選取原因"})
  )
...
if user["role"] == "會計":
  ...
  allowed_regions = list(
      dict.fromkeys(
          [CLINIC_TO_REGION.get(c, "屏東") for c in allowed_clinics]
      )
  )
  ...
  clinic_all_df = (
      staff_df[staff_df["院所"] == selected_clinic]
      if not staff_df.empty
      else pd.DataFrame()
  )
  cur_total = len(clinic_all_df)
  cur_comp = (
      len(clinic_all_df[clinic_all_df["符合資格"] == "🟢 符合"])
      if cur_total > 0
      else 0
  )
  ...
  next_df = (
      clinic_all_df[clinic_all_df["本月離職"] == False]
      if cur_total > 0
      else pd.DataFrame()
  )
  next_total = len(next_df)
  next_comp = (
      len(next_df[next_df["符合資格"] == "🟢 符合"]) if next_total > 0 else 0
  )
  ...
  nk2.metric(
      "下月預估符合人數",
      f"{next_comp} 人",
      delta=f"-{cur_comp - next_comp} 人" if cur_comp > next_comp else None,
  )
  ...
  st.markdown("---")
  with st.expander(
      f"📄 上傳【{selected_clinic}】健保署/衛福部「醫事人員執業清冊"
      " (.xls/.xlsx)」雙向對帳與比對",
      expanded=False,
  ):
    st.write(
        "上傳衛福部清冊後，系統會自動與系統母數進行【雙向比對】："
    )
    clinic_prsn_file = st.file_uploader(
        f"選擇 [{selected_clinic}] 執業清冊 (.xls / .xlsx)",
        type=["xls", "xlsx"],
        key="clinic_prsn_uploader",
    )
    ...
    if clinic_prsn_file is not None:
      try:
        ...
        if (
            "執業類別" in uploaded_prsn_df.columns
            and "姓名" in uploaded_prsn_df.columns
        ):
          nurses_in_file = uploaded_prsn_df[
              uploaded_prsn_df["執業類別"].isin(
                  ["護理師", "護士", "護理人員"]
              )
          ].copy()
          file_names = set(nurses_in_file["姓名"].tolist())
          sys_names = (
              set(clinic_all_df["姓名"].tolist())
              if not clinic_all_df.empty
              else set()
          )
          nurses_in_file["比對狀態"] = nurses_in_file["姓名"].apply(
              lambda x: (
                  "✅ 已在名冊中" if x in sys_names else "🆕 新執登人員 (系統缺)"
              )
          )

          st.info(
              f"解析到執業清冊共有 **{len(nurses_in_file)}**"
              " 位護理人員。清冊比對明細："
          )
          display_cols = [
              c
              for c in ["姓名", "執業類別", "執業起日", "比對狀態"]
              if c in nurses_in_file.columns
          ]
          ...
          if extra_in_sys:
            st.warning(
                "⚠️ **【系統母數異常警示】** 發現有"
                f" **{len(extra_in_sys)}**"
                " 位系統母數同仁在最新的衛福部清冊中【找不到名字】（疑已離職/退保/異動）："
            )
            st.write(
                "疑已退保/離職人員名單：",
                ", ".join([f"**{n}**" for n in extra_in_sys]),
            )

          new_nurses = nurses_in_file[
              nurses_in_file["比對狀態"] == "🆕 新執登人員 (系統缺)"
          ]
          if not new_nurses.empty:
            st.warning(
                f"偵測到有 **{len(new_nurses)}**"
                " 位「🆕 新執登人員」尚未建立於系統名冊中。"
            )
            if st.button(
                f"🚀 一鍵將 {len(new_nurses)} 位新執登護理師同步匯入至"
                f" [{selected_clinic}] 名冊"
            ):
              new_rows = []
              ...
              merged_staff_df = pd.concat(
                  [staff_df, pd.DataFrame(new_rows)], ignore_index=True
              )
              save_data(merged_staff_df)
              st.balloons()
              st.success(
                  f"🎉 成功同步！已為 [{selected_clinic}] 新增"
                  f" {len(new_nurses)} 位護理人員！"
              )
              st.rerun()
  ...
  st.markdown("---")
  st.write(
      "✏️ **選擇「符合原因」自動連動資格，若本月有離職請勾選「本月離職」框框：**"
  )

  editable_df = (
      staff_df[staff_df["院所"] == selected_clinic].copy()
      if not staff_df.empty
      else pd.DataFrame()
  )
  edited_df = st.data_editor(
      ...
      column_config={
          ...
          "身份": st.column_config.SelectboxColumn(
              "身份別", options=["舊有員工", "新進人員"], required=True
          ),
          "到職日": st.column_config.TextColumn("到職日"),
          "符合原因": st.column_config.SelectboxColumn(
              "符合原因", options=REASON_OPTIONS, required=True
          ),
          "符合資格": st.column_config.TextColumn(
              "符合資格 (自動判定)", disabled=True
          ),
          "本月離職": st.column_config.CheckboxColumn(
              "本月離職 (勾選則下月扣除)", default=False
          ),
  ...
  with st.expander("🗑️ 批量移除/刪除已離職人員名單"):
    current_names = (
        editable_df["姓名"].tolist()
        if not editable_df.empty and "姓名" in editable_df.columns
        else []
    )
    if current_names:
      remove_names = st.multiselect(
          "選擇要從系統永久移除的人員：", current_names
      )
      if st.button("❌ 確認移除選取的人員"):
        if remove_names:
          new_staff_df = staff_df[
              ~(
                  (staff_df["院所"] == selected_clinic)
                  & (staff_df["姓名"].isin(remove_names))
              )
          ]
          save_data(new_staff_df)
      ...
      if st.form_submit_button("新增該人員"):
        if new_name:
          add_row = {
              ...
              "到職日": new_date,
              "符合原因": new_reason,
              "符合資格": get_compliance_status(new_reason),
              "本月離職": False,
              "備註": new_memo,
          }
          updated_all = pd.concat(
              [staff_df, pd.DataFrame([add_row])], ignore_index=True
          )
...
# ===================================================================
  ...
  csv_data = (
      staff_df.to_csv(index=False).encode("utf-8-sig")
      if not staff_df.empty
  ...
  with st.expander("📂 展開母數清冊上傳與預審面板", expanded=False):
    prsn_file = st.file_uploader(
        "選擇全網護理人員 Excel 檔案 (.xlsx / .xls / .csv)",
        type=["xlsx", "xls", "csv"],
        key="master_prsn_uploader",
    )
    if prsn_file is not None:
      try:
        imported_df = (
            pd.read_csv(prsn_file)
            if prsn_file.name.endswith(".csv")
            else pd.read_excel(prsn_file)
        )
        if (
            "中文姓名" in imported_df.columns
            and "上班地點" in imported_df.columns
        ):
          st.success(
              "✅ 解析完成！您可直接在下方表格修正【歸類院】或單筆刪除錯誤資料："
          )
          imported_df["歸類院"] = imported_df["上班地點"].apply(
              normalize_clinic_name
          )
          imported_df["歸類區域"] = imported_df["歸類院"].map(
              lambda x: CLINIC_TO_REGION.get(x, "高雄")
          )
          ...
          edited_preview_df = st.data_editor(
              ...
              column_config={
                  ...
                  "到職日": st.column_config.TextColumn("到職日"),
                  "上班地點": st.column_config.TextColumn(
                      "原始上班地點", disabled=True
                  ),
                  "歸類院": st.column_config.SelectboxColumn(
                      "歸類院 (可手動修正)",
                      options=ALL_CLINICS,
                      required=True,
                  ),
                  "歸類區域": st.column_config.TextColumn(
                      "歸類區域 (自動對應)", disabled=True
                  ),
                  "備註": st.column_config.TextColumn("備註"),
              },
              use_container_width=True,
          ...
          edited_preview_df["歸類區域"] = edited_preview_df["歸類院"].map(
              lambda x: CLINIC_TO_REGION.get(x, "高雄")
          )
          import_mode = st.radio(
              "選擇匯入方式：",
              [
                  "增量比對更新（保留既有勾選資料，僅新增新到職人員）",
                  "完全覆蓋資料庫（以新檔案為準重新建立全網母數）",
              ],
          )
          ...
          if st.button("🚀 確認匯入此預審名單"):
            ...
            if import_mode.startswith("完全覆蓋"):
              ...
              st.balloons()
              st.success(
                  f"🎉 成功覆蓋！已將 **{len(parsed_new_df)}** 位護理人員匯入！"
              )
            else:
              existing_pairs = (
                  set(zip(staff_df["院所"], staff_df["姓名"]))
                  if not staff_df.empty
                  else set()
              )
              added_rows = [
              ...
              if added_rows:
                merged_df = pd.concat(
                    [staff_df, pd.DataFrame(added_rows)], ignore_index=True
                )
                save_data(merged_df)
                st.balloons()
                st.success(
                    "🎉 增量更新成功！自動新增了"
                    f" **{len(added_rows)}** 位護理人員！"
                )
              else:
  ...
  with st.expander("📋 展開離職同仁名冊與一鍵標記面板", expanded=True):
    ...
    with col_res_img:
      ...
      res_img_file = st.file_uploader(
          "可上傳最新離職通報截圖 (.jpg / .png)",
          type=["jpg", "png", "jpeg"],
          key="res_img_uploader",
      )
      if res_img_file is not None:
        st.image(
            res_img_file, caption="最新離職人員通報清單", use_container_width=True
        )
      else:
        st.info(
            "💡 提示：可上傳離職截圖對照，或直接參考右側自動整理之清單。"
        )
    ...
    with col_res_tbl:
      ...
      if st.button(
          "⚡ 一鍵為上述護理同仁「勾選本月離職並寫入備註離職日」"
      ):
        updated_count = 0
        if not staff_df.empty:
          for item in resigned_nurses_data:
            mask = (staff_df["院所"] == item["院所"]) & (
                staff_df["姓名"] == item["姓名"]
            )
            if mask.any():
          ...
          if updated_count > 0:
            ...
            st.warning(
                "⚠️ 系統名冊中尚未找到對應人員，請確認是否已匯入該院人員母數。"
            )
        else:
  ...
  filter_region = st.selectbox(
      "選擇要檢視的區域（或顯示全部）：",
      ["全部區域"] + list(CLINIC_REGIONS.keys()),
  )

  summary_list = []
  display_clinics = (
      ALL_CLINICS
      if filter_region == "全部區域"
      else CLINIC_REGIONS[filter_region]
  )

  for c in display_clinics:
    c_region = CLINIC_TO_REGION.get(c, "")
    c_all_df = (
        staff_df[staff_df["院所"] == c]
        if not staff_df.empty and "院所" in staff_df.columns
        else pd.DataFrame()
    )
    cur_tot = len(c_all_df)
    cur_comp = (
        len(c_all_df[c_all_df["符合資格"] == "🟢 符合"]) if cur_tot > 0 else 0
    )
    cur_req = math.ceil(cur_tot / 2) if cur_tot > 0 else 0
    cur_stat = (
        "🟢 達標"
        if (cur_comp >= cur_req and cur_tot > 0)
        else ("⚪ 無資料" if cur_tot == 0 else "🔴 未達標")
    )

    next_df = (
        c_all_df[c_all_df["本月離職"] == False]
        if cur_tot > 0
        else pd.DataFrame()
    )
    next_tot = len(next_df)
    next_comp = (
        len(next_df[next_df["符合資格"] == "🟢 符合"]) if next_tot > 0 else 0
    )
    next_req = math.ceil(next_tot / 2) if next_tot > 0 else 0
    next_stat = (
        "🟢 預估達標"
        if (next_comp >= next_req and next_tot > 0)
        else ("⚪ 無資料" if next_tot == 0 else "🔴 預估未達標")
    )
  ...
  hr_view_region = col_hr_detail_r.selectbox(
      "1. 選擇大項【區域】：", list(CLINIC_REGIONS.keys()), key="hr_view_reg"
  )
  hr_view_clinic = col_hr_detail_c.selectbox(
      "2. 選擇小項【院】：",
      CLINIC_REGIONS[hr_view_region],
      key="hr_view_cli",
  )

  st.markdown(
      f"#### 📍【{hr_view_region}區】{hr_view_clinic} - 現況與下月預估卡控"
  )

  hr_clinic_all_df = (
      staff_df[staff_df["院所"] == hr_view_clinic]
      if not staff_df.empty
      else pd.DataFrame()
  )
  hr_cur_tot = len(hr_clinic_all_df)
  hr_cur_comp = (
      len(hr_clinic_all_df[hr_clinic_all_df["符合資格"] == "🟢 符合"])
      if hr_cur_tot > 0
      else 0
  )
  ...
  hr_next_df = (
      hr_clinic_all_df[hr_clinic_all_df["本月離職"] == False]
      if hr_cur_tot > 0
      else pd.DataFrame()
  )
  hr_next_tot = len(hr_next_df)
  hr_next_comp = (
      len(hr_next_df[hr_next_df["符合資格"] == "🟢 符合"])
      if hr_next_tot > 0
      else 0
  )
  ...
  nk1.metric(
      ...
      f"{hr_next_tot} 人",
      delta=(
          f"-{hr_cur_tot - hr_next_tot} 人離職"
          if hr_cur_tot > hr_next_tot
          else None
      ),
  )
  nk2.metric(
      "下月預估符合人數",
      f"{hr_next_comp} 人",
      delta=f"-{hr_cur_comp - hr_next_comp} 人" if hr_cur_comp > hr_next_comp else None,
  )
  ...
  st.markdown("---")
  with st.expander(
      f"📄 上傳【{hr_view_clinic}】健保署/衛福部「醫事人員執業清冊"
      " (.xls/.xlsx)」雙向對帳與比對",
      expanded=False,
  ):
    st.write(
        "上傳衛福部清冊後，系統會自動與【"
        f"{hr_view_clinic}】現有母數進行【雙向比對】："
    )
    hr_clinic_prsn_file = st.file_uploader(
        f"選擇 [{hr_view_clinic}] 執業清冊 (.xls / .xlsx)",
        type=["xls", "xlsx"],
        key="hr_clinic_prsn_uploader",
    )
    ...
    if hr_clinic_prsn_file is not None:
      try:
        ...
        if (
            "執業類別" in hr_uploaded_prsn_df.columns
            and "姓名" in hr_uploaded_prsn_df.columns
        ):
          hr_nurses_in_file = hr_uploaded_prsn_df[
              hr_uploaded_prsn_df["執業類別"].isin(
                  ["護理師", "護士", "護理人員"]
              )
          ].copy()
          hr_file_names = set(hr_nurses_in_file["姓名"].tolist())
          hr_sys_names = (
              set(hr_clinic_all_df["姓名"].tolist())
              if not hr_clinic_all_df.empty
              else set()
          )
          ...
          st.info(
              f"解析到執業清冊共有 **{len(hr_nurses_in_file)}**"
              " 位護理人員。清冊比對明細："
          )
          hr_display_cols = [
              c
              for c in ["姓名", "執業類別", "執業起日", "比對狀態"]
              if c in hr_nurses_in_file.columns
          ]
          st.dataframe(
              hr_nurses_in_file[display_cols], use_container_width=True
          )

          hr_extra_in_sys = [
              name for name in hr_sys_names if name not in hr_file_names
          ]
          if hr_extra_in_sys:
            st.warning(
                "⚠️ **【系統母數異常警示】** 發現有"
                f" **{len(hr_extra_in_sys)}**"
                " 位系統母數同仁在最新的衛福部清冊中【找不到名字】（疑已離職/退保/異動）："
            )
            st.write(
                "疑已退保/離職人員名單：",
                ", ".join([f"**{n}**" for n in hr_extra_in_sys]),
            )

          hr_new_nurses = hr_nurses_in_file[
              hr_nurses_in_file["比對狀態"] == "🆕 新執登人員 (系統缺)"
          ]
          if not hr_new_nurses.empty:
            st.warning(
                f"偵測到有 **{len(hr_new_nurses)}**"
                " 位「🆕 新執登人員」尚未建立於系統名冊中。"
            )
            if st.button(
                f"🚀 一鍵將 {len(hr_new_nurses)} 位新執登護理師同步匯入至"
                f" [{hr_view_clinic}] 名冊",
                key="hr_sync_btn",
            ):
              new_rows = []
  ...
  hr_editable_df = (
      staff_df[staff_df["院所"] == hr_view_clinic].copy()
      if not staff_df.empty
      else pd.DataFrame()
  )
  if not hr_editable_df.empty:
    ...
    if st.button(
        f"💾 儲存 [{hr_view_clinic}] 的人員變更", key="hr_save_btn"
    ):
      other_clinics_df = (
          staff_df[staff_df["院所"] != hr_view_clinic]
          if not staff_df.empty
      ...
      hr_edited_df["本月離職"] = (
          hr_edited_df["本月離職"].fillna(False).astype(bool)
      )
      hr_edited_df["符合原因"] = (
          hr_edited_df["符合原因"]
          .fillna("⚠️ 請選取原因")
      ...
      hr_edited_df["符合資格"] = hr_edited_df["符合原因"].map(
          get_compliance_status
      )
      hr_edited_df["區域"] = hr_view_region
      hr_edited_df["院所"] = hr_view_clinic
      new_staff_df = pd.concat(
          [other_clinics_df, hr_edited_df], ignore_index=True
      )
      save_data(new_staff_df)
    ...
    with st.expander(
        f"🗑️ 批量移除/刪除 [{hr_view_clinic}] 已離職人員名單"
    ):
      hr_current_names = (
          hr_editable_df["姓名"].tolist()
          if not hr_editable_df.empty and "姓名" in hr_editable_df.columns
          else []
      )
      if hr_current_names:
        hr_remove_names = st.multiselect(
            "選擇要從系統永久移除的人員：",
            hr_current_names,
            key="hr_multiselect_del",
        )
        if st.button("❌ 確認移除選取的人員", key="hr_btn_del"):
          if hr_remove_names:
            new_staff_df = staff_df[
                ~(
                    (staff_df["院所"] == hr_view_clinic)
                    & (staff_df["姓名"].isin(hr_remove_names))
                )
            ]
            save_data(new_staff_df)
        ...
        if st.form_submit_button("新增該人員"):
          if hr_new_name:
            add_row = {
                ...
                "到職日": hr_new_date,
                "符合原因": hr_new_reason,
                "符合資格": get_compliance_status(hr_new_reason),
                "本月離職": False,
                "備註": hr_new_memo,
            }
            updated_all = pd.concat(
    ...
    st.info(
        f"💡 目前【{hr_view_clinic}】尚未建立人員名冊，可利用上方按鈕上傳執業清冊或全醫療網護理人員"
        " Excel 母數總表。"
    )
  ...
  unpassed = (
      summary_df[summary_df["下月卡控預測"] == "🔴 預估未達標"][
          "院所名稱"
      ].tolist()
    ...
    if st.button("🚀 私訊發送 LINE 提醒"):
      if line_token and accountant_line_id:
        success = send_line_message(
            line_token, accountant_line_id, msg_template
        )
        if success:
          st.success(
              f"✅ 已成功私訊發送給 [{selected_notify_clinic}] 負責會計！"
          )
        else:
